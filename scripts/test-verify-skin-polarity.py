...
def main():
    ...
    with tempfile.TemporaryDirectory() as raw:
        ...
        color_cases = (
            ("#17152b80", ((23.0, 21.0, 43.0), 128.0 / 255.0)),
            ("#abc8", ((170.0, 187.0, 204.0), 136.0 / 255.0)),
            ("rgb(23 21 43 / 16%)", ((23.0, 21.0, 43.0), 0.16)),
        ...
        for rank, alpha in LIGHT_RAMP:
            ...
            replacement = 'data-share="{}" fill="rgb(23 21 43 / {}%)"'.format(
        ...
        paperless = paperless.replace('<rect width="100%" height="100%" fill="#ffffff"/>', "", 1)
        ...
        on_mask = light_source.replace(
            '<rect x="576" y="40" width="252" height="252" rx="2" fill="#ffffff"/>',
            '<rect x="576" y="40" width="252" height="252" rx="2" data-share="18.29" fill="#ffffff"/>',
            1,
        ).replace(
            '<rect x="576" y="40" width="252" height="252" rx="2" data-share="18.29" '
            'fill="rgba(23,21,43,0.16)"',
            '<rect x="576" y="40" width="252" height="252" rx="2" fill="rgba(23,21,43,0.16)"',
            1,
        )
    ...
    accent_in_ramp = [member for member in ramp if member.ink != (23.0, 21.0, 43.0)]
    if accent_in_ramp:
        failures.append(
    ...
    light_strong = RELATIVE_LUMINANCE(COMPOSITE((23.0, 21.0, 43.0), 0.16, light_paper))
    light_weak = RELATIVE_LUMINANCE(COMPOSITE((23.0, 21.0, 43.0), 0.04, light_paper))
    dark_strong = RELATIVE_LUMINANCE(COMPOSITE((246.0, 245.0, 250.0), 0.14, dark_paper))
    dark_weak = RELATIVE_LUMINANCE(COMPOSITE((246.0, 245.0, 250.0), 0.04, dark_paper))
