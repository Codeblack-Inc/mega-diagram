...
ASSETS = ROOT / "skills/mega-diagram/assets"
GOOD = ASSETS / "example-sankey.html"
REFERENCE = ROOT / "skills/mega-diagram/references/type-sankey.md"
...
RIBBON = ('<path d="M132,120 C316,120 316,120 500,120 L500,224 '
          'C316,224 316,224 132,224 Z" fill="rgba(104,101,123,0.18)"/>')
RIBBON_ANCHOR = '<path d="M132,120 C316,120 316,120 500,120'
RIBBON_FILL = 'fill="rgba(104,101,123,0.18)"'
...
SWATCH = 'fill="rgba(104,101,123,0.18)" stroke="#68657b" stroke-width="1"'
# The Failed node bar — last stage, 32px for 1,600 min at 50 min/px.
BAR = '<rect x="880" y="344" width="12" height="32" fill="#17152b"/>'
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as raw:
        ...
        case(
            ...
            re.sub(r'[ ]*<rect x="\d+" y="\d+" width="12" height="\d+" fill="#17152b"/>\n', '', source),
        ...
        case(
            ...
            source.replace(ANNOT,
                '      <rect x="600" y="300" width="16" height="8" fill="#ffffff"/>\n'
                + ANNOT, 1),
        ...
        case(
            ...
            source.replace(ANNOT,
                '      <rect x="600" y="496" width="12" height="8" fill="#17152b"/>\n'
                + ANNOT, 1),
            source, "neither a node bar",
        ...
        dashed = source.replace(
            anchor,
            '      <path d="M140,64 L640,48" fill="none" stroke="#17152b" '
        ...
        case(
            ...
            source.replace(anchor,
                '      <path d="M140,64 L640,48 Z" fill="rgba(23,21,43,0.13)"/>\n' + anchor, 1),
            source, "neither a readable ribbon",
        ...
        for name, frag, expect, why in (
            ("fat-stroke",
             '<path d="M140,64 L640,48" fill="none" stroke="#17152b" stroke-width="40"/>',
             "paints", "an unfilled path drawn as a 40px band"),
            ("fat-dashed",
             '<path d="M140,64 L640,48" fill="none" stroke="#17152b" stroke-width="40" stroke-dasharray="4 3"/>',
             "paints", "a dashed unfilled path too wide to be a hairline"),
            ("undashed",
             '<path d="M140,64 L640,48" fill="none" stroke="#17152b" stroke-width="1"/>',
        ...
        classed = source.replace(
            anchor,
            '      <path class="fat" d="M140,64 L640,48" fill="none" stroke="#17152b" '
            'stroke-width="1" stroke-dasharray="4 3"/>\n' + anchor, 1
        ...
        case(
            ...
            source.replace(
                RIBBON_FILL + "/>",
                RIBBON_FILL + ' stroke="#ffffff" stroke-width="30"/>', 1),
            source, "paints",
        ...
        for name, repl in (
            ...
            ("inv-alpha0", 'fill="rgba(104,101,123,0)"'),
        ):
            case(
                failures, directory, name,
        ...
        malformed = (
            (RIBBON_FILL, 'fill="rgba(104,101,123,zz)"', "a non-numeric rgba alpha"),
        ...
        for dash, dashed in (
            ...
            frag = (f'<path d="M140,64 L640,48" fill="none" stroke="#17152b" '
        ...
        if RIBBON not in source:
            ...
            for name, mutated, expect, describe in (
                ...
                ("aud-singlequote", altered(RIBBON_FILL,
                                            "fill='rgba(104,101,123,0.18)' style='stroke-width:30'"),
        ...
        if BAR not in source:
            ...
            for name, mutated, reject, describe in (
                ("bar-nofill", rebar('fill="#17152b"', 'fill="none"'), True,
                 "a node bar that paints no fill"),
                ("bar-transparent", rebar('fill="#17152b"', 'fill="transparent"'), True,
                 "a node bar painted transparent"),
                ("bar-alpha-zero", rebar('fill="#17152b"', 'fill="rgba(23,21,43,0)"'), True,
                 "a node bar whose fill alpha is zero"),
                ("bar-fill-opacity", rebar('fill="#17152b"',
                                           'fill="#17152b" fill-opacity="0"'), True,
        ...
        for name, mutated, reject, describe in (
            ...
            ("len-px", rebar('width="12"', 'width="12px"'), False,
             "a bar width in px"),
            ("paint-bad-fn", rebar('fill="#17152b"', 'fill="rgba(23,21,43,zz)"'), True,
             "a bar fill whose alpha cannot be read"),
            ("paint-bad-fo", rebar('fill="#17152b"',
                                   'fill="#17152b" fill-opacity="abc"'), True,
             "a bar fill-opacity that is not a number"),
            ("paint-hex5", rebar('fill="#17152b"', 'fill="#2d314"'), True,
             "a five-digit hex, which is not a colour"),
            ("paint-hex4", rebar('fill="#17152b"', 'fill="#2d31"'), False,
             "a four-digit hex, which is RGBA and legal"),
            ("paint-hex8-zero", rebar('fill="#17152b"', 'fill="#2d314200"'), True,
        ...
        (sandbox / "skills/mega-diagram/assets").mkdir(parents=True)
        (sandbox / "skills/mega-diagram/references").mkdir(parents=True)
        (sandbox / "scripts").mkdir()
        shutil.copy(CHECKER, sandbox / "scripts/verify-sankey.py")
        shutil.copy(REFERENCE, sandbox / "skills/mega-diagram/references" / REFERENCE.name)
        for variant in sorted(ASSETS.glob("example-sankey*.html")):
            shutil.copy(variant, sandbox / "skills/mega-diagram/assets" / variant.name)
        ...
        dark = sandbox / "skills/mega-diagram/assets/example-sankey-dark.html"
        ...
        doc = sandbox / "skills/mega-diagram/references" / REFERENCE.name
        original_doc = doc.read_text(encoding="utf-8")
        excerpt = '<rect x="500" y="120" width="12" height="104" fill="#17152b"/>'
        ...
        (sandbox / "skills/mega-diagram/assets" / "example-sankey-full.html").unlink()
        code, output = invoke(sandboxed)
        if code == 0:
