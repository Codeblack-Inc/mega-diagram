...
INK = "rgba(23,21,43,0.55)"
ACCENT_FILL = "rgba(47,111,219,0.55)"
...
def dot(value: float, row: int = 0, name=None, drawn_cx=None, drawn_cy=None,
        r: float = R, fill: str = INK, stroke: str = "#68657b",
...
def focal(value: float, row: int = 0, name=None, **kw) -> str:
    return dot(value, row, name, fill=ACCENT_FILL, stroke="#2f6fdb", **kw)
...
def run_cases(h: Harness) -> int:
    ...
    h.expect_finding(
        ...
        document(swarm(23), dot(300, fill="rgba(23,21,43,0.30)"), TICKS),
    ...
    h.expect_finding(
        ...
        document(swarm(22), focal(300),
                 dot(320, fill="rgba(110,155,234,0.55)", stroke="#6e9bea"),
                 TICKS),
        r"one accent dot max",
    )
    ...
    h.expect_clean(
        ...
        document(swarm(23), dot(300, name="req-a"), TICKS,
                 label("req-a", cx(300),
                       extra='style="line-height:1.4;color:#17152b"')),
    )
    h.expect_clean(
