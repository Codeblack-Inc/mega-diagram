...
SKILL = ROOT / "skills/mega-diagram/SKILL.md"
GALLERY = ROOT / "skills/mega-diagram/assets/index.html"
ASSET_DIR = ROOT / "skills/mega-diagram/assets"
README = ROOT / "README.md"
HIGH_LEVEL_REFERENCE = ROOT / "skills/mega-diagram/references/type-high-level.md"
ONBOARDING_REFERENCE = ROOT / "skills/mega-diagram/references/onboarding.md"
...
def check_routing_surfaces(errors: list[str], root: Path) -> None:
    for relative, reference_link in ROUTING_SURFACES.items():
        reference = root / "skills/mega-diagram" / reference_link
        if not reference.is_file():
            errors.append(f"routing source of truth is missing: skills/mega-diagram/{reference_link}")
        path = root / relative
        if not path.is_file():
...
def check_factory_install_surface(errors: list[str], root: Path) -> None:
    ...
    architecture_blocks = [
        block
        for block in code_blocks
        if "mega-diagram/" in block and "commands/" in block
    ]
...
TYPE_SIZE_SURFACES = (
    ("skills/mega-diagram/assets", ("example-", "template")),
    ("skills/mega-diagram/references", ("type-",)),
)
...
SKILL_PACKAGE = Path("skills/mega-diagram")
