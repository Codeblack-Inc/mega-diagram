...
STYLE_GUIDE = ROOT / "skills/mega-diagram/references/style-guide.md"
...
def google_fonts_families(url):
    ...
    if url.strip() == PRETENDARD_STYLESHEET:  # mega: pinned Pretendard (Hangul)
        return {"pretendard"}
    try:
        parsed = urlparse(url)
        port = parsed.port
...
def parse_args():
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="lint every skills/mega-diagram/assets/example-*.html and template*.html file",
    )
    parser.add_argument(
        "--baseline",
        action="store_true",
