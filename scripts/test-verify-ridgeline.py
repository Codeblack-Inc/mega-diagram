...
GOOD = ROOT / "skills/mega-diagram/assets/example-ridgeline.html"
...
RIDGELINES = sorted((ROOT / "skills/mega-diagram/assets").glob("example-ridgeline*.html"))
...
FOCAL_PATH = ('<path data-ridge="checkout-api" data-baseline="320" '
              ...
              + FOCAL_D +
              ' fill="rgba(47,111,219,0.16)" stroke="#2f6fdb" stroke-width="2.4" '
...
REPORT_RULE = ('<line data-ridge="report-export" data-role="baseline" x1="320" y1="376" '
               'x2="680" y2="376" stroke="rgba(23,21,43,0.25)" stroke-width="1"/>')
...
def synthetic(n_ridges, n_bins, top=100, pitch=40, amp=2.0, x0=200, dx=20, unit=10):
    ...
    for s in range(n_ridges):
        ...
        parts.append('<line data-ridge="r%d" data-role="baseline" x1="%d" y1="%g" '
                     'x2="%d" y2="%g" stroke="rgba(23,21,43,0.25)" stroke-width="1"/>'
                     % (s, xs[0], baseline, xs[-1], baseline))
        parts.append('<path data-ridge="r%d" data-baseline="%g" data-bins="%s" d="%s" '
                     'fill="rgba(23,21,43,0.12)" stroke="%s" stroke-width="%s"/>'
    ...
    with tempfile.TemporaryDirectory() as raw:
        ...
        case(
            ...
            source.replace('      <text data-ridge="checkout-api" data-role="range" '
                           'x="696" y="323.5" fill="#68657b" font-size="9" '
        ...
        case(
            ...
            source.replace('stroke="rgba(23,21,43,0.80)" stroke-width="1.2"',
                           'stroke="#2f6fdb" stroke-width="2.4"', 1),
        ...
        for sibling in ("example-slopegraph.html", "example-bump.html",
            ...
            other = ROOT / "skills/mega-diagram/assets" / sibling
