#!/usr/bin/env python3
"""One-shot environment diagnostics verifier for mega-diagram.
...
SKILL_RELATIVE = Path("skills/mega-diagram/SKILL.md")
        ...
        return (
            CheckResult(
                ...
                message=(
                    ...
                    "mega-diagram requires Python >= 3.10."
                ),
                fix="Upgrade Python to 3.10+ and re-run /mega-diagram:doctor.",
            ),
            command_name,
        )
...
def check_common_path_mistakes(root: Path, cwd: Path) -> CheckResult:
    ...
    if resolve_skill_file(root) is None:
        warnings.append(
            "mega-diagram SKILL.md was not found under the resolved installation root."
        )
        fixes.append("Reinstall or update mega-diagram, then run the doctor again.")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run one-shot mega-diagram environment diagnostics."
    )
    parser.add_argument(
        "--strict",
        action="store_true",
