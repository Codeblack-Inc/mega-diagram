...
<text x="10" y="90" fill="rgba(23,21,43,0.06)" font-size="72" font-family="'Geist Mono', monospace">2026</text>
"""
...
def font_fixture():
    ...
    with tempfile.TemporaryDirectory(prefix="verify-docs-sync-fonts-") as temp_dir:
        ...
        for relative in FONT_SURFACES:
            target = root / "skills/mega-diagram" / relative
            target.parent.mkdir(parents=True, exist_ok=True)
            target.write_bytes((ROOT / "skills/mega-diagram" / relative).read_bytes())
        yield root
...
def check_font_link_parity(verify) -> None:
    ...
    with font_fixture() as root:
        skill = root / "skills/mega-diagram"
...
def check_title_font_link(verify) -> None:
    ...
    with font_fixture() as root:
        mutate(
            root / "skills/mega-diagram/assets/template-motion.html",
            NOTO_SERIF_FAMILY,
            "",
        )
        errors = run_checks(
    ...
    with font_fixture() as root:
        motion = root / "skills/mega-diagram/assets/template-motion.html"
    ...
    with font_fixture() as root:
        for relative in FONT_SURFACES:
            ...
            mutate(root / "skills/mega-diagram" / relative, family, "")
        errors = run_checks(root, verify.check_export_font_parity)
        if errors:
...
def main() -> int:
    ...
    line_dark = (
        ROOT / "skills/mega-diagram/assets/example-line-dark.html"
    ...
    verify.check_line_dark_skin(
        errors,
        line_dark.replace("--color-paper:#17152b", "--color-paper:#ffffff", 1),
    )
    expected = (
    ...
    errors = []
    verify.check_type_ramp(
        errors, GRID_SKILL.replace("rgba(23,21,43,0.06)", "rgba(23,21,43,0.30)"), TYPE_RAMP_SPEC
    )
    ...
    with tempfile.TemporaryDirectory(prefix="legacy-type-sizes-") as temp_dir:
        ...
        assets = fake_root / "skills/mega-diagram/assets"
        references = fake_root / "skills/mega-diagram/references"
...
"""
        ...
        profile_reference = root / "skills/mega-diagram/references/profiles.md"
        doctor_reference = root / "skills/mega-diagram/references/doctor.md"
        export_reference = root / "skills/mega-diagram/references/export.md"
        drawio_reference = root / "skills/mega-diagram/references/import-drawio.md"
        mermaid_reference = root / "skills/mega-diagram/references/import-mermaid.md"
        excalidraw_reference = (
            root / "skills/mega-diagram/references/import-excalidraw.md"
        )
        ...
        factory_manifest.write_text(
            json.dumps(
                {
                    "name": "mega-diagram",
                    "repository": "https://github.com/example/mega-diagram",
                }
            ),
            encoding="utf-8",
        )
        factory_marketplace.write_text(
            json.dumps({"name": "mega-diagram"}),
            encoding="utf-8",
        )
        valid_readme = """# mega-diagram

```bash
droid plugin marketplace add https://github.com/example/mega-diagram
...
"""
        ...
        expected = (
            ...
            "`droid plugin marketplace add https://github.com/example/mega-diagram` "
            "then `droid plugin install mega-diagram@mega-diagram`"
        )
        if errors != [expected]:
            raise AssertionError(
        ...
        readme.write_text(
            valid_readme.replace(
                "mega-diagram@mega-diagram", "mega-diagram@wrong-marketplace"
            ),
            encoding="utf-8",
        )
        errors = []
