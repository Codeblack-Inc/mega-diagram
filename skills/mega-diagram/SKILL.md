...
Create diagrams as self-contained HTML files with inline SVG and an editorial design system — tuned for Korean documents. Fork of [diagram-design](https://github.com/cathrynlavery/diagram-design) (Cathryn Lavery, MIT) with the mega skin, Korean-first labels, and transparent output.
...
## 0. First-time setup — skin

The shipped mega skin (paper `#ffffff`, ink `#17152b`, accent `#2f6fdb`) is the intended default — draw with it without asking.

First resolve any project `.mega-diagram` marker per [`references/profiles.md`](references/profiles.md); a resolved marker selects its profile. Customize [`references/style-guide.md`](references/style-guide.md) only when the user names another brand or destination: branch per [`references/onboarding.md`](references/onboarding.md) (website URL, installed skill, local design system, pasted tokens) or load a saved profile, including the `mega-ppt` and `mono` presets in [mega.md](references/mega.md#document-presets). After onboarding, offer to save it as a named profile.
...
**The design system is skinnable.** All colors, typography, and tokens live in a single source of truth — [`references/style-guide.md`](references/style-guide.md). This file describes semantic roles (`paper`, `ink`, `muted`, `accent`, `link`, …). The default skin is the mega palette (white paper, mega ink, mega-diagram blue accent, mega muted, violet links); to apply your own brand, either edit `style-guide.md` directly or run the URL-based flow described in [`references/onboarding.md`](references/onboarding.md).
...
**Korean labels** — Hangul resolves to Pretendard via `'Geist', 'Pretendard', 'Noto Sans KR', …`; see [Korean](references/style-guide.md#korean-labels). Also: [Chinese](references/style-guide.md#traditional-chinese-labels), [Cyrillic](references/style-guide.md#cyrillic-labels).
...
**Default: transparent.** Draw no full-bleed background `<rect>` — the HTML `body` supplies `paper`, and an exported SVG/PNG sits on whatever the destination document already is. Masks (label rects, node paper masks) still fill with `paper`, which is white because Korean documents are white; for a non-white destination switch the skin per [mega.md](references/mega.md#document-presets). Don't wrap the diagram in a secondary container background.

**Optional: dotted paper variant** — a 22×22 `<pattern>` of `r=0.9` dots in `ink @ 0.10`, drawn as one `fill="url(#dots)"` rect at opacity 0.6. Hero diagrams on a dedicated page only; never for documents or slides.

### Arrow markers (define all three, always)

```svg
<marker id="arrow" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="#68657b"/>
</marker>
<marker id="arrow-accent" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="#2f6fdb"/>
</marker>
<marker id="arrow-link" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="#6043d5"/>
</marker>
```

| Arrow | Stroke | When |
|---|---|---|
| Default | muted `#68657b` | Internal, generic |
| Accent | blue `#2f6fdb` | Primary / highlighted / headline |
| Link-blue | `#6043d5` | HTTP/API calls, external systems |
...
<text x="CX" y="CY+2" fill="#17152b" font-size="12" font-weight="600"
      font-family="'Geist', sans-serif" text-anchor="middle">Node Name</text>
<!-- 5. Technical sublabel (Geist Mono) -->
<text x="CX" y="CY+18" fill="#68657b" font-size="9"
...
<text x="MID_X" y="ARROW_Y-11" fill="#8a879c" font-size="8"
...
<line x1="30" y1="LEGEND_Y-8" x2="VIEWBOX_W-30" y2="LEGEND_Y-8"
      stroke="rgba(23,21,43,0.10)" stroke-width="0.8"/>
<text x="30" y="LEGEND_Y+8" fill="#68657b" font-size="8" font-family="'Geist Mono', monospace"
...
- `border: 1px solid rgba(23,21,43,0.12)`
- `border-radius: 6px`, `padding: 1.25rem`
- **No `box-shadow`**
- Card dots: 7px, `border-radius: 50%` — ink / muted / coral (= accent) / link / soft variants
...
- [ ] Every arrow label has an opaque `fill="#ffffff"` rect behind it?
...
|---|---|---|
| **Minimal light** (default) | `assets/template.html`, `example-<type>.html` | Document-ready. Diagram + title. Transparent SVG. |
