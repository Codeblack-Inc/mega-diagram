...
# mega: the one approved non-Google stylesheet — Pretendard, the mega Hangul face.
...
def reference_error(tag: str, rel: str, value: str) -> str | None:
    ...
    if tag == "link" and "stylesheet" in rel.casefold().split():
        if is_approved_google_fonts_stylesheet(stripped) or stripped == PRETENDARD_STYLESHEET:
            return None
