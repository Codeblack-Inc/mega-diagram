...
Load this file when the user asks to run diagnostics, health checks, or first-run troubleshooting, or when they invoke `/mega-diagram:doctor` or `/doctor`.

The goal is a one-shot report that checks local readiness for mega-diagram import/export and command routing, without mutating user files or installing dependencies.

Resolve the mega-diagram installation from this loaded reference, not from the
...
- Re-run: /mega-diagram:doctor --strict
```
