...
focal_accent: "#2f6fdb"             # one color for all focal components (default = SKILL accent)
dark: false
```
...
Bars span the full zone width minus 16-px padding on each side. Bars marked `focal: true` use `bar_h_focal=56` and accent styling (fill `rgba(focal_accent, 0.08)`, stroke `focal_accent`). Non-focal bars use `bar_h_default=44` with muted styling (fill `rgba(23,21,43,0.05)`, stroke `rgba(23,21,43,0.32)`).
...
All side-column nodes use fixed `w=160 h=64`. Same fill / stroke pattern: fill `rgba(104,101,123,0.06)`, stroke `#8a879c`, stroke-width 1.
...
|---|---|---|---|---|---|
| `primary` | `#2f6fdb` (focal_accent) | 1.4 | — | `arrow-accent` | Every edge whose endpoint is a `focal: true` component. Also every Trino → consumer edge (serve-flow rule). |
| `secondary` | `#68657b` (muted) | 1.2 | — | `arrow` | Default for internal platform-component edges and source → platform edges that don't touch focal. |
| `federated` | `#6043d5` (link-blue) | 1.0 | `4,3` | `arrow-link` | Federation queries (e.g., source DB → Trino). |
| `trigger` | `#68657b` (muted) | 1.0 | `4,3` | `arrow` | Every edge originating from a `kind: bar` component (Airflow drops). Unlabelled. |
| `auth` | `#2f6fdb` | 1.2 | `5,4` | `arrow-accent` | Every edge from a footer node up to the zone bottom edge. **Never to a specific component.** |
...
<defs>
  ...
  <marker id="arrow-link"   markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#6043d5"/></marker>
...
| Token | Light | Dark |
|---|---|---|
| Page paper | `#ffffff` | `#17152b` |
| Ink | `#17152b` | `#ffffff` |
| Muted | `#68657b` | `#d4d2de` |
| Accent | `#2f6fdb` | `#6e9bea` |
| Link (federated) | `#6043d5` | `#9a86ec` |
| Side-column fill | `rgba(104,101,123,0.06)` | `rgba(246,245,250,0.06)` |
| Side-column stroke | `#8a879c` | `rgba(246,245,250,0.30)` |
| Zone fill | `rgba(23,21,43,0.025)` | `rgba(246,245,250,0.04)` |
| Zone stroke | `rgba(23,21,43,0.32)` | `rgba(246,245,250,0.30)` |
| Non-focal bar fill | `rgba(23,21,43,0.05)` | `rgba(246,245,250,0.06)` |
