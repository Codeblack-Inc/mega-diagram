...
- **Y-axis gridlines:** 4–6 horizontal lines at regular intervals. Stroke `rgba(23,21,43,0.08)` (very faint), 0.8px. X-axis baseline at `rgba(23,21,43,0.25)`, 1px.
...
<!-- Bar body -->
<rect x="X" y="Y" width="W" height="H" fill="rgba(104,101,123,0.15)" stroke="#68657b" stroke-width="1"/>
<!-- Value label above bar -->
<text x="X+W/2" y="Y-8" fill="#68657b" font-size="8" font-family="'Geist Mono', monospace" text-anchor="middle">VALUE</text>
```

Focal bar: replace fill with `rgba(47,111,219,0.12)`, stroke with `#2f6fdb`, label fill with `#2f6fdb`.
...
- **Gridlines:** vertical at each tick, `rgba(23,21,43,0.08)` 0.8px, spanning y 56→408. At the domain floor the axis line replaces the gridline rather than doubling it: `rgba(23,21,43,0.25)` 1px, y 40→420.
...
- **Connector:** `rgba(23,21,43,0.55)` 1px, declared before both dots so the dots cap it. It is not the axis hairline: the connector is what says *these two dots are one row*, so it has to clear 3:1 (0.55 gives 3.19:1; the 0.25 the axis uses gives 1.60:1).
...
<line x1="458" y1="96" x2="740" y2="96" stroke="rgba(23,21,43,0.55)" stroke-width="1"/>
<circle cx="458" cy="96" r="6" fill="#ffffff" stroke="#68657b" stroke-width="1.5"/>
<circle cx="740" cy="96" r="6" fill="#2f6fdb" stroke="#17152b" stroke-width="1"/>
<text x="446" y="100" fill="#68657b" font-size="8" font-family="'Geist Mono', monospace" text-anchor="end">34</text>
<text x="752" y="100" fill="#68657b" font-size="8" font-family="'Geist Mono', monospace">71</text>
<text x="188" y="100" fill="#17152b" font-size="11" font-weight="600" font-family="'Geist', sans-serif" text-anchor="end">Platform</text>
```
...
**Dark theme.** Dots and labels swap as expected — hollow fill `#17152b` with `#d4d2de` stroke, solid `#6e9bea` with an `#ffffff` stroke, labels `#d4d2de`. **The hairlines must invert too:** gridlines `rgba(246,245,250,0.08)`, axis `rgba(246,245,250,0.20)`, connector `rgba(246,245,250,0.40)` — the connector again heavier than the axis, for the same 3:1 reason. `rgba(23,21,43,…)` *is* the dark paper colour at every alpha value, so a connector, gridline, or axis carried over from light composites to exactly 1.000:1 — the gap encoding and the scale both vanish.
