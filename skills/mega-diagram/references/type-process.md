...
- Dot pattern: 22×22 grid, `circle r=0.8`, `fill rgba(23,21,43,0.10)`. Opacity 0.55.
- Alternating lane tints: odd-indexed lanes (0, 2, …) receive `rgba(23,21,43,0.018)` fill from `x=140` to `viewBox_w`.
- Lane dividers: horizontal hairlines at every `lane_y_top(k)` and at `legend_y_top`; stroke `rgba(23,21,43,0.12)` width 0.8.
- Label column right border: vertical hairline at `x = label_col_w`, stroke `rgba(23,21,43,0.20)` width 1, from `y = header_h` to `y = legend_y_top`.
...
- Default fill: `rgba(23,21,43,0.12)`, number text ink.
- Focal fill: `rgba(47,111,219,0.20)`, number text accent (§5).
...
- Default fill: muted (`#68657b` light / `#d4d2de` dark).
- Focal fill: accent (`#2f6fdb` light / `#6e9bea` dark).
...
|---|---|---|---|---|---|
| `normal` | `#68657b` (muted) | 1.0 | — | `arrow` | Standard data hand-off between steps or actors. Unlabelled. |
| `focal-in` / `focal-out` | `#2f6fdb` (accent) | 1.2 | — | `arrow-accent` | Every edge whose endpoint is the focal node (`focal-in`) or origin is the focal node (`focal-out`). |
| `trigger` | `#68657b` (muted) | 1.0 | `4,3` | `arrow-sm` | Orchestration trigger (scheduler → tool, manual override → upstream step). Unlabelled. |
...
<defs>
  <pattern id="dots" width="22" height="22" patternUnits="userSpaceOnUse">
    <circle cx="11" cy="11" r="0.8" fill="rgba(23,21,43,0.10)"/>
  </pattern>
...
| Lane tint | `rgba(23,21,43,0.018)` | `rgba(246,245,250,0.025)` |
| Dividers | `rgba(23,21,43,0.12)` | `rgba(246,245,250,0.12)` |
| Label col divider | `rgba(23,21,43,0.20)` | `rgba(246,245,250,0.22)` |
| Default chip fill | `rgba(23,21,43,0.12)` | `rgba(246,245,250,0.12)` |
| Focal chip fill | `rgba(47,111,219,0.20)` | `rgba(110,155,234,0.22)` |
| Default node fill | white | `rgba(246,245,250,0.04)` |
| Default node stroke | `rgba(23,21,43,0.25)` | `rgba(246,245,250,0.20)` |
| Focal node fill | `rgba(47,111,219,0.08)` | `rgba(110,155,234,0.12)` |
| Focal node stroke | `#2f6fdb` | `#6e9bea` |
