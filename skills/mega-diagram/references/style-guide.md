...
**The single source of truth for colors, typography, and tokens.** Every diagram draws from this — not from hex values inlined in other reference files. If you want to change the visual skin of mega-diagram, change this file.

Default skin is the mega palette — white paper, mega ink, mega-diagram blue accent, mega muted, violet links. It's designed to look good out of the box; swap these values (or run [`onboarding.md`](onboarding.md)) and every new diagram inherits the new skin without touching any type-specific logic.
...
|---|---|---|---|
| `paper` | Destination background, label masks, default node fill | `#ffffff` (white) | `#17152b` (mega ink) |
| `paper-2` | Diagram container bg, secondary fill | `#f6f5fa` (mega paper) | `#24223a` |
| `ink` | Primary text, primary stroke | `#17152b` (mega ink) | `#f6f5fa` (mega paper) |
| `ink-strong` | High-contrast text on warm accent fills | `#111111` | `#111111` |
| `muted` | Secondary text, default arrow stroke | `#68657b` (mega muted) | `#d4d2de` |
| `soft` | Sublabels, boundary labels | `#8a879c` | `#9d9ab0` |
| `rule` | Hairline borders | `rgba(23,21,43,0.12)` | `rgba(246,245,250,0.12)` |
| `rule-solid` | Stronger borders, baselines | `#d4d2de` | `rgba(212,210,222,0.25)` |
| `accent` | Focal / 1–2 max per diagram | `#2f6fdb` (mega-diagram blue) | `#6e9bea` |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(47,111,219,0.08)` | `rgba(110,155,234,0.10)` |
| `link` | HTTP/API calls, external arrows | `#6043d5` (mega violet) | `#9a86ec` |

> **Brand palette source:** this skin is the [mega BI](https://github.com/Codeblack-Inc/mega-bi) palette — ink `#17152B`, paper `#F6F5FA`, muted `#68657B`, violet `#6043D5` — plus the mega-diagram product blue `#2F6FDB` as the one accent. Violet (the master-brand color) takes the `link` role so the accent stays the only blue on the page. `paper` is pure white on purpose: Korean documents (HWP, Word, 보고서 PPT) are white, and every label mask is filled with `paper`, so a white paper makes the masks invisible when the diagram is placed on the page. See [mega.md](mega.md) for presets that match mega-ppt decks and black-and-white print.

> **Background:** the diagram `<svg>` carries **no full-bleed background rect**. The page colour comes from the HTML `body`, so an exported SVG or PNG is transparent and sits on whatever the destination document already is.

### Inversion rule (light → dark)

Any `rgba(23,21,43, X)` in light becomes `rgba(246,245,250, X)` in dark. Same opacities, RGB flipped. The accent gets a slight hue-shift brighter to read on dark paper.
...
| `terminal-ink` | `#f5f5f5` | Primary text, primary stroke (white-smoke) |
...
```svg
<text font-family="'Geist', 'Pretendard', 'Noto Sans KR', 'Apple SD Gothic Neo', 'Malgun Gothic', sans-serif">결제 서비스</text>
```

**Pretendard first.** Pretendard is the mega Hangul face (mega-ppt and mega-ui use it too). The templates load it from one pinned jsDelivr stylesheet next to the Google Fonts link — the only non-Google stylesheet `self_check.py` accepts:
...
Geist still draws Latin; Hangul falls through to Pretendard, then to Noto Sans KR if the CDN is unreachable. Writing rules for Korean documents (명사형, 단위, 문서 프리셋) live in [mega.md](mega.md).
...
- **Paper matches the destination**: `paper` fills every label mask, so it must equal the document the diagram lands on. The mega skin uses pure white because Korean documents are white; a brand that ships on cream paper sets `paper` to that cream.
