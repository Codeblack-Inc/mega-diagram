...
<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="8"
      fill="rgba(23,21,43,0.02)" stroke="rgba(23,21,43,0.10)" stroke-width="0.8"/>
<rect x="{label_x}" y="{y+4}" width="{label_w}" height="12" rx="2" fill="{paper}"/>
<text x="{label_cx}" y="{y+13}" fill="rgba(23,21,43,0.40)" font-size="7"
...
- Zone fill: `rgba(23,21,43,0.02)` (2% ink wash). Any stronger competes with node fills.
- Max 3 zones per diagram. More and it reads like a swimlane (use that type instead).
- Dark mode: swap `rgba(23,21,43,…)` → `rgba(246,245,250,…)` same opacities; label mask fill = `paper` (dark).
